...
This code defines two classes: `Rectangle` and `Square`. `Rectangle` represents a rectangle with a given width and height, and `Square` represents a square with a given side length. Both classes have various methods for calculating properties of the shapes, such as area, perimeter, and diagonal length.

### Usage

To use these classes, first import the code into your project. Then, you can create new `Rectangle` and `Square` objects as follows:
...
You can then use the various methods of the objects to calculate properties of the shapes:

```
my_rectangle.get_area()   # returns 200
my_square.get_perimeter() # returns 20

```

You can also set the width, height, or side length of the objects using the appropriate methods:

```
my_rectangle.set_width(30)
my_square.set_side(8)
